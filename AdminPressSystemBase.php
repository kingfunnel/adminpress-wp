<?php 
 ...
 /* Encrypted code is not available in lite version. Please upgrade to the pro version*/
if(!class_exists("AdminPressSystemBase")) {
	class AdminPressSystemBase {
    	public $key = "105F1EF7287A3322";
    	private $product_id = "1";
    	private $product_base = "adminpress-wp";
    	private $server_host = "https://up.kingfunnel.co/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_adminpress-wp_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=adminpress-wp_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
